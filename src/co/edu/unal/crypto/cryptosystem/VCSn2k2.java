 ...
 */
public class VCSn2k2 implements VisualCryptosystem<BufferedImage> {
    ...
    public BufferedImage[] encrypt(BufferedImage secret) {
        ...
        WritableRaster[] shares = {share1.getRaster(), share2.getRaster()};
    ...
    public BufferedImage decrypt(BufferedImage[] shares) {
        ...
        WritableRaster secretRaster = secret.getRaster();
        WritableRaster[] rasters = {shares[0].getRaster(), shares[1].getRaster()};
    ...
    private void permut(int arr[], int k) {
        ...
        if (k == arr.length) {
            int[][] p = new int[SZ][SZ];
            BinarySubPixel[] bBins = new BinarySubPixel[2];
            BinarySubPixel[] wBins = new BinarySubPixel[2];
            ...
            bBins[0] = new BinarySubPixel(SZ, p);
            wBins[0] = new BinarySubPixel(SZ, p);
            wBins[1] = new BinarySubPixel(SZ, p);
            
            p[0][0] = 1-pattern[arr[0]];
            p[0][1] = 1-pattern[arr[1]];
            p[1][0] = 1-pattern[arr[2]];
            p[1][1] = 1-pattern[arr[3]];
            bBins[1] = new BinarySubPixel(SZ, p);
